...
def write_rating_train3(path,filename,train_triple,entity2id):
    str_rating = ''
    str_rating+='place'+','+'person'+','+'rating'+'\n'
    for triple in train_triple:
...
def get_person2place(train_triple_name,kg1):
    company2place = {}
    person2company = {}
    person2place = set()
    for triple in train_triple_name:
        company2place[triple[0]] = triple[1]
    for triple in kg1:
        if triple[0] not in person2company:
            person2company[triple[0]] = []
            person2company[triple[0]].append(triple[1])
        else:
            person2company[triple[0]].append(triple[1])
    for key,item in person2company.items():
        for company in item:
            if company in company2place:
                person2place.add((key,company2place[company]))
    return person2place
def get_test_kg1_info(path,filename):
    company2P = set()
    with open(path + filename, encoding='utf-8') as file:
        next(file)
        lines = file.readlines()
...
if __name__=='__main__':
    ...
    kg1_train = get_test_kg1_info(path, 'kg1_train_D.txt')
    kg1_test = get_test_kg1_info(path, 'kg1_test.txt')
    ...
    person2place_train = get_person2place(train_triple_name, kg1_train)
    person2place_test = get_person2place(test_triple_name, kg1_test)

    person_set = set()
    place_set = set()
    for two in person2place_train:
        person_set.add(two[0])
        place_set.add(two[1])
    for two in person2place_test:
        person_set.add(two[0])
        place_set.add(two[1])
    ...
    write_rating_train3(path,'/rating_train_static.csv',person2place_train,entity2id)
    # write_rating_train1(path, '/rating_test.csv', test_triple_name, entity2id, Sc2c)
    write_rating_train3(path, '/rating_test.csv', person2place_test, entity2id)

    write_person2id(path,'/place2id.csv',place_set,entity2id)

    write_person2id(path,'/person2id.csv',person_set,entity2id)
